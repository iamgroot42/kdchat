...
string encrypt(string data, string pass_key, string init_vec){
  unsigned char *plaintext, *key, *iv;
  plaintext = (unsigned char*)data.c_str();
  key = (unsigned char*)pass_key.c_str();
  iv = (unsigned char*)init_vec.c_str();
  unsigned char ciphertext[BUFFER_SIZE * 16];
  EVP_CIPHER_CTX *ctx;
  int len, ciphertext_len, plaintext_len = data.length();
  ...
  string return_string((const char*)ciphertext);
  return return_string;
}

string decrypt(string data, string pass_key, string init_vec){
  ...
  EVP_CIPHER_CTX *ctx;
  int len, plaintext_len, ciphertext_len = data.length();
  ctx = EVP_CIPHER_CTX_new();
  ...
  string return_string((const char*)plaintext);
  return return_string;
}

string senti_decrypt(unsigned char *ciphertext, int ciphertext_len, unsigned char *key,
...
int main(){
    ...
    unsigned char decrypted_text[50];
    unsigned char *a,*b,*c;
    string A("raisins"), B("zoha"), C("0123456789123456");
    a = (unsigned char *)A.c_str();
    b = (unsigned char *)B.c_str();
    c = (unsigned char *)C.c_str();
    string enc = encrypt(A,B,C);
    string dec = decrypt(enc,B,C);
    cout<<dec<<endl;
    return 0;
    // cout<<"Encrypted :"<<encr<<endl;
    // string decr = decrypt(encr, "zoha", iv);
